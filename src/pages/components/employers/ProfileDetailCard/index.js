...
const item = (title, value) => {
  return (
    <Box sx={{ p: 1 }}>
      ...
      <Typography sx={{ 
        textAlign: 'justify',
        color: (theme) => value ? theme.palette.text.primary : theme.palette.grey[400],
  ...
  return (
    <>
      <Button
        ...
        fullWidth={matches}
        startIcon={sentEmail ? <MarkEmailReadRoundedIcon color='white'/> : <ForwardToInboxIcon />}
      >
            ...
            <Stack spacing={3}>
              ...
              >
                ...
                >
                  ...
                  <Stack flex={1} alignItems={{ xs: 'center', sm: 'flex-start' }}>
                    ...
                    >
                      Cập nhật: {dayjs(profileDetail?.updateAt).format('DD/MM/YYYY')}
                    </Typography>
                    {profileDetail?.lastViewedDate && (
                      <Chip
                        ...
                        label={`Xem lần cuối: ${dayjs(profileDetail?.lastViewedDate).format('DD/MM/YYYY HH:mm')}`}
                        size="small"
                      />
                    )}
                  </Stack>
              ...
              >
                ...
                >
                  ...
                  />
                  <Button
                    variant={profileDetail?.isSaved ? "contained" : "outlined"}
                    size="medium"
                    startIcon={profileDetail?.isSaved ? <FavoriteIcon /> : <FavoriteBorderIcon />}
                ...
                {/* CV đính kèm section */}
                {profileDetail?.type && profileDetail.type === CV_TYPES.cvUpload && (
                  <Card
                    variant="outlined"
                    sx={{
                      p: 2,
                  ...
                  >
                    ...
                    >
                      <Stack 
                        ...
                        width={{ xs: '100%', sm: 'auto' }}
                      >
                        <FontAwesomeIcon icon={faFilePdf} color="#d32f2f" size="lg" />
                      ...
                      >
                        <Button
                          ...
                          onClick={() => downloadPdf(profileDetail?.fileUrl, profileDetail?.title)}
                          fullWidth={matches}
                        >
                          Tải xuống
                        </Button>
                ...
                <Stack spacing={4}>
                  ...
                  <Box>
                    ...
                    <Grid container>
                      ...
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Số điện thoại', profileDetail?.jobSeekerProfile?.phone)}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Giới tính', allConfig?.genderDict[profileDetail?.jobSeekerProfile?.gender])}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Ngày sinh', dayjs(profileDetail?.jobSeekerProfile?.birthday).format('DD/MM/YYYY'))}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Tình trạng hôn nhân', allConfig?.maritalStatusDict[profileDetail?.jobSeekerProfile?.maritalStatus])}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Tỉnh/Thành phố', allConfig?.cityDict[profileDetail?.jobSeekerProfile?.location?.city])}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Quận/Huyện', profileDetail?.jobSeekerProfile?.location?.districtDict?.name)}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Địa chỉ', profileDetail?.jobSeekerProfile?.location?.address)}
                      </Grid>
                    </Grid>
                  </Box>
                  ...
                  <Box>
                    ...
                    <Grid container>
                      ...
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Cấp bậc mong muốn', allConfig?.positionDict[profileDetail?.position])}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Trình độ học vấn', allConfig?.academicLevelDict[profileDetail?.academicLevel])}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Kinh nghiệm', allConfig?.experienceDict[profileDetail?.experience])}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Nghề nghiệp', allConfig?.careerDict[profileDetail?.career])}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Địa điểm làm việc', allConfig?.cityDict[profileDetail?.city])}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Mức lương mong muốn', salaryString(profileDetail?.salaryMin, profileDetail?.salaryMax))}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Nơi làm việc', allConfig?.typeOfWorkplaceDict[profileDetail?.typeOfWorkplace])}
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        {item('Hình thức làm việc', allConfig?.jobTypeDict[profileDetail?.jobType])}
                      </Grid>
                    </Grid>
                  </Box>
                  ...
                  <Box>
                    ...
                    >
                      <Typography sx={{
                        color: profileDetail?.description ? defaultTheme.palette.text.primary : defaultTheme.palette.text.italic.color,
                        fontStyle: profileDetail?.description ? 'normal' : 'italic',
                        fontSize: profileDetail?.description ? '0.95rem' : defaultTheme.palette.text.italic.fontSize,
                        lineHeight: 1.6
                      }}>
